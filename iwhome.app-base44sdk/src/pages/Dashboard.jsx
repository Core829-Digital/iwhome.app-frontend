...
export default function Dashboard() {
  ...
  const checkAuth = async () => {
    try {
      ...
      // Check if user needs welcome modal (first login)
      if (!currentUser.welcome_email_sent) {
        setShowWelcome(true);
      }
      
      // Check for company code
      if (currentUser.company_code === 'IWSHOWROOMAZIENDE@AREAPRIVATA') {
        await base44.auth.updateMe({ 
          is_company: true,
          company_role: currentUser.company_role || 'admin'
        });
      }
      
      // Check for upcoming appointments and send notifications
      const upcomingAppts = await base44.entities.Appointment.filter({
        ...
        if (aptDate.toDateString() === tomorrow.toDateString()) {
          ...
          if (existingNotif.length === 0) {
            await base44.entities.Notification.create({
              ...
              message: `Hai un appuntamento domani alle ${apt.appointment_time}`,
  ...
  const { data: quotes = [], refetch: refetchQuotes } = useQuery({
    queryKey: ['quotes', statusFilter, typeFilter, dateFilter],
    queryFn: async () => {
      let query = {};
      if (statusFilter !== 'all') query.status = statusFilter;
      if (typeFilter !== 'all') query.quote_type = typeFilter;
      
      const allQuotes = await base44.entities.Quote.filter(query, '-created_date');
      
      if (dateFilter === 'today') {
        const today = new Date().toISOString().split('T')[0];
        return allQuotes.filter(q => q.created_date.startsWith(today));
      } else if (dateFilter === 'week') {
        const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
        return allQuotes.filter(q => q.created_date >= weekAgo);
      }
  ...
  const { data: appointments = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      return await base44.entities.Appointment.filter({ email: user.email }, '-appointment_date');
    },
    enabled: !!user,
    refetchInterval: 5000,
    refetchOnWindowFocus: true,
  ...
  const { data: documents = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      return await base44.entities.Document.filter({ created_by: user.email });
    },
    enabled: !!user,
    refetchInterval: 5000,
    refetchOnWindowFocus: true,
      ...
      <div className="lg:ml-[280px] pt-[76px] relative z-10 min-h-screen pb-safe">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-[#dee2e6]">Benvenuto, {user.full_name}</p>
            </div>
            {activeDevices > 1 && (
              <motion.div
