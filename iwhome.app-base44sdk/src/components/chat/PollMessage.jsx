            ...
            <CardHeader className="pb-2 p-4">
                <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-blue-400 uppercase tracking-wider">Sondaggio</span>
                    </div>
                    {isClosed && (
                ...
                <div className="space-y-3">
                        ...
                        return (
                            <div key={option} className="space-y-2">
                                ...
                                >
                                    ...
                                    {(isClosed || userVote) && (
                                        <div
                                            className={`absolute inset-0 opacity-10 ${isAccept ? 'bg-green-500' : (isNegotiation ? 'bg-blue-500' : 'bg-[#adb5bd]')}`}
                                    ...
                                    <div className="relative p-3.5 flex items-center justify-between z-10">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-2 h-2 rounded-full ${isAccept ? 'bg-green-500' : (option === 'Rifiuto' ? 'bg-red-500' : (option === 'Controproposta' ? 'bg-blue-500' : 'bg-gray-500'))}`} />
                                        ...
                                        <div className="space-y-3">
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    placeholder="0.00"
                                                    className="w-full bg-[#16191c] rounded-lg border border-[#495057] text-white text-sm p-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                                                />
                                            </div>
                                            <div>
                                                ...
                                                <textarea
                                                    ...
                                                    className="w-full bg-[#16191c] rounded-lg border border-[#495057] text-white text-sm p-2.5 focus:ring-2 focus:ring-blue-500 outline-none transition-all min-h-[80px]"
                                                />
                                            </div>
                                            <div className="flex justify-end gap-2 pt-1">
                                                ...
                                                <Button
                                                    ...
                                                    className={`h-8 text-xs font-bold px-4 ${option === 'Rifiuto' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}
                                                >
                    ...
                    {isAdmin && !isClosed && poll.votes?.map((vote, idx) => {
                        if (vote.proposed_price) {
                            return (
                                <div key={idx} className="mt-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl animate-in fade-in">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs font-bold text-blue-400 uppercase tracking-wider">Controproposta Ricevuta</span>
